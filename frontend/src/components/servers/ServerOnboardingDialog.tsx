      ...
      <div className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-2xl border border-line bg-panel p-6 shadow-2xl">
        ...
        {!active ? (
          ...
          <div className="mt-6">
            ...
            <CredentialFields authMethod={form.auth_method} setAuthMethod={(value) => update('auth_method', value)} privateKey={privateKey} setPrivateKey={setPrivateKey} password={password} setPassword={setPassword} />
            {(active.status === 'pending' || active.status === 'failed' || active.status === 'rollback_required' || active.status === 'cancelled') && <div className="mt-5 flex flex-wrap gap-2"><button type="button" disabled={!hasCredentials || busy} onClick={() => { void retry() }} className="inline-flex items-center gap-2 rounded-xl border border-line px-4 py-2.5 text-xs font-semibold text-copy disabled:opacity-50"><RefreshCw size={14} />{active.status === 'pending' ? 'Resume' : 'Retry'}</button><button type="button" disabled={!hasCredentials || busy} onClick={() => { void onboarding.rollback.mutateAsync({ id: active.id, payload: credentials() }) }} className="rounded-xl border border-warning/30 px-4 py-2.5 text-xs font-semibold text-warning disabled:opacity-50">Rollback managed changes</button></div>}
            ...
            <div className="mt-5"><button type="button" disabled={onboarding.diagnose.isPending || !hasCredentials} onClick={() => { void onboarding.diagnose.mutateAsync({ id: active.id, payload: credentials() }) }} className="inline-flex items-center gap-2 rounded-xl border border-line px-4 py-2.5 text-xs font-semibold text-copy disabled:opacity-50"><ShieldCheck size={14} />Diagnose</button>{diagnosis && <div className="mt-3 rounded-xl border border-line bg-panel-raised p-4 text-xs"><p className="font-semibold text-copy">Diagnosis: {diagnosis.status}</p><p className="mt-2 text-muted">Node Exporter {diagnosis.node_exporter} · Prometheus {diagnosis.prometheus} · Firewall {diagnosis.firewall} · Network {diagnosis.network} · Latency {diagnosis.latency_ms ?? '—'} ms</p>{diagnosis.errors.length > 0 && <ul className="mt-2 list-disc pl-4 text-warning">{diagnosis.errors.map((item) => <li key={item}>{item}</li>)}</ul>}</div>}</div>
          </div>
        )}
      </div>
    </div>
...
function CredentialFields({ authMethod, setAuthMethod, privateKey, setPrivateKey, password, setPassword }: { authMethod: OnboardingStartRequest['auth_method']; setAuthMethod: (value: OnboardingStartRequest['auth_method']) => void; privateKey: string; setPrivateKey: (value: string) => void; password: string; setPassword: (value: string) => void }): React.JSX.Element {
  return <div className="grid gap-3 rounded-xl border border-warning/20 bg-warning/5 p-4 sm:col-span-2"><p className="text-xs text-warning">Credentials are not retained after starting a job. Re-enter them for retry, rollback or diagnosis.</p><select value={authMethod} onChange={(event) => setAuthMethod(event.target.value as OnboardingStartRequest['auth_method'])} className="rounded-xl border border-line bg-panel-raised px-3 py-2.5 text-xs text-copy"><option value="private_key">Private key</option><option value="password">Temporary password</option></select>{authMethod === 'private_key' ? <textarea value={privateKey} onChange={(event) => setPrivateKey(event.target.value)} rows={4} className="rounded-xl border border-line bg-panel-raised p-3 font-mono text-xs text-copy" autoComplete="off" placeholder="Paste the temporary private key" /> : <input type="password" value={password} onChange={(event) => setPassword(event.target.value)} className="rounded-xl border border-line bg-panel-raised px-3 py-2.5 text-xs text-copy" autoComplete="new-password" placeholder="Enter the temporary password" />}</div>
}
