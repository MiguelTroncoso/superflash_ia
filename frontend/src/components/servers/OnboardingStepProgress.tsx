  ...
  return (
    <div>
      <div className="flex items-center justify-between text-xs">
        ...
        <span className="text-muted">{progress}% · estimated time ~2 min</span>
      </div>
      ...
      <div className="mt-6 grid gap-2 sm:grid-cols-3">
        {steps.map((step, index) => {
          ...
          const isComplete = index <= completedIndex || step.key === 'completed'
          return (
            <div key={step.key} className={`rounded-xl border p-3 text-xs ${isActive ? 'border-brand bg-brand/10 text-copy' : isComplete ? 'border-success/30 text-success' : 'border-line text-muted'}`}>
              ...
              <span className="mt-1 block">{isComplete ? 'Complete' : isActive ? 'In progress' : 'Pending'}</span>
            </div>
          )
        })}
      </div>
